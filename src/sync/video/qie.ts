...
class QiEVideoUploader {
  ...
  private async clickLocalUploadTab(): Promise<boolean> {
    try {
      ...
      // 查找页面选项卡（用于区分）
      const pageTabLabels = document.querySelectorAll(
        ".omui-tab__label:not(.omui-dialog .omui-tab__label)",
  ...
  private async uploadCover(coverDataUrl: string): Promise<boolean> {
    try {
      ...
      if (!uploadTabClicked) {
        ...
        // 尝试直接点击本地上传按钮
        const directUploadButton = (await this.waitForElement(
      ...
      if (uploadSuccess) {
        ...
        const nextButton = (await this.waitForElement(
          ".omui-button.next-btn, .omui-button--primary",
          5000,
        )) as HTMLElement;
        if (nextButton?.textContent?.includes("下一步")) {
